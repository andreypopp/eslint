...
var esprima = require("esprima-fb"),
    estraverse = require("estraverse-jsx"),
    ...
    api.verify = function(text, config, filename, saveState) {
        ...
        if (ast) {
            ...
            Object.keys(config.rules).filter(function(key) {
                ...
                if (ruleCreator) {
                    try {
                        rule = ruleCreator(new RuleContext(key, api, options, filename));
            ...
            currentScopes = escope.analyze(ast, { ignoreEval: true, traverse: estraverse.traverse }).scopes;
